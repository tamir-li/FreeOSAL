...
#define    OSAL_SIGNAL_QUEUE_SIZE                             (32)  //2-order
#define    OSAL_REALTIME_SIGNAL_QUEUE_SIZE                    (16)  //2-order

#define    OSAL_SIGNAL_CACHE_QUEUE_SIZE                       (4)  //2-order
#define    OSAL_REALTIME_SIGNAL_CACHE_QUEUE_SIZE              (4)  //2-order
...
#define    OSAL_SIGNAL_EMIT_ISR(isr_signal, isr_parm)                                                                                    \
    do{                                                                                                                                  \
        ...
        if(OSAL_FIFO_NEXT_INDEX(g_osal_signalfifo_ctl.front, OSAL_SIGNAL_QUEUE_SIZE) != g_osal_signalfifo_ctl.rear)                      \
        {                                                                                                                                \
            g_osal_signalfifo[g_osal_signalfifo_ctl.front].signal = (isr_signal);                                                        \
            g_osal_signalfifo[g_osal_signalfifo_ctl.front].parm = (isr_parm);                                                            \
...
#define    OSAL_REALTIME_SIGNAL_EMIT_ISR(isr_rt_signal, isr_rt_parm)                                                                     \
    do{                                                                                                                                  \
        ...
        {                                                                                                                                \
            g_osal_rt_signalfifo[g_osal_rt_signalfifo_ctl.front].signal = (isr_rt_signal);                                               \
            g_osal_rt_signalfifo[g_osal_rt_signalfifo_ctl.front].parm = (isr_rt_parm);                                                   \
...
#define    OSAL_FLUSH_SIGNAL_ISR()                                                                                                       \
    do{                                                                                                                                  \
        ...
        if(g_osal_signalfifo_cache_ctl.front != g_osal_signalfifo_cache_ctl.rear)                                                        \
        {                                                                                                                                \
            if(OSAL_FIFO_NEXT_INDEX(g_osal_signalfifo_ctl.front, OSAL_SIGNAL_QUEUE_SIZE) != g_osal_signalfifo_ctl.rear)                  \
            {                                                                                                                            \
                g_osal_signalfifo[g_osal_signalfifo_ctl.front] = g_osal_signalfifo_cache[g_osal_signalfifo_cache_ctl.rear];              \
                g_osal_signalfifo_ctl.front = OSAL_FIFO_NEXT_INDEX(g_osal_signalfifo_ctl.front, OSAL_SIGNAL_QUEUE_SIZE);                 \
...
#define    OSAL_FLUSH_REALTIME_SIGNAL_ISR()                                                                                              \
    do{                                                                                                                                  \
        ...
        if(g_osal_rt_signalfifo_cache_ctl.front != g_osal_rt_signalfifo_cache_ctl.rear)                                                  \
        {                                                                                                                                \
            if(OSAL_FIFO_NEXT_INDEX(g_osal_rt_signalfifo_ctl.front, OSAL_REALTIME_SIGNAL_QUEUE_SIZE) != g_osal_rt_signalfifo_ctl.rear)   \
            {                                                                                                                            \
                g_osal_rt_signalfifo[g_osal_rt_signalfifo_ctl.front] = g_osal_rt_signalfifo_cache[g_osal_rt_signalfifo_cache_ctl.rear];  \
                g_osal_rt_signalfifo_ctl.front = OSAL_FIFO_NEXT_INDEX(g_osal_rt_signalfifo_ctl.front, OSAL_REALTIME_SIGNAL_QUEUE_SIZE);  \
